...
test('mobile exposes stable ids for Android smoke', async () => {
  ...
  assert.match(source, /const MOBILE_TAB_TEST_IDS: Record<ProductSurfaceId, string> = \{/)
  assert.match(source, /chat: 'chat-tab'/)
  assert.match(source, /dm: 'dm-tab'/)
  assert.match(source, /people: 'people-tab'/)
  assert.match(source, /treehole: 'treehole-tab'/)
  assert.match(source, /testID=\{MOBILE_TAB_TEST_IDS\[surface\.id\]\}/)
