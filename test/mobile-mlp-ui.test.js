...
test('mobile primary empty states live in checked TSX components', async () => {
  ...
  assert.match(emptyComponents, /from '\.\.\/src\/direct-chat-empty-copy\.ts'/)
  assert.match(emptyComponents, /copy = getDirectChatEmptyCopy\(\)/)
  assert.match(directComponents, /from '\.\.\/src\/direct-chat-empty-copy\.ts'/)
  assert.match(directComponents, /const emptyMessageCopy = getDirectChatEmptyCopy\(\{/)
  assert.match(directComponents, /relationshipState: requestTarget\?\.relationshipState/)
  assert.match(
    homeChatPane,
...
test('mobile direct contact chips and revoke actions expose trust state', async () => {
  ...
  assert.match(profileComponents, /createProfileDetailState/)
  assert.match(contactProfileDetail, /const profileDetailState = createProfileDetailState\(/)
  assert.match(contactProfileDetail, /const relationshipActions = profileDetailState\.actions/)
...
test('mobile Chat and Contacts share one profile detail route', async () => {
  ...
  assert.match(selectionModel, /selectProfileSelectionSource/)
  assert.match(selectionModel, /kind: 'trusted'/)
  assert.match(selectionModel, /kind: 'incoming_request'/)
  assert.match(selectionModel, /kind: 'outgoing_request'/)
  assert.match(selectionModel, /kind: 'blocked'/)
  assert.match(selectionModel, /kind: 'request_target'/)
  assert.match(contactManager, /<ContactProfileDetail[\s\S]*profile=\{selectedProfile\}/)
  assert.match(contactProfileDetail, /label=\{profile\.messageLabel\}/)
  assert.match(contactProfileDetail, /label=\{profile\.enterHomeLabel\}/)
...
test('mobile setup actions share one icon button component', async () => {
  ...
  assert.match(quickStart, /label='Add friend'/)
  assert.match(roomComponents, /onOpenContacts=\{\(\) => onTabChange\('people'\)\}/)
  assert.match(quickStart, /Show My QR or add a friend\./)
  assert.doesNotMatch(quickStart, /Show My QR or open your home\./)
  assert.match(quickStart, /label='Open Home'/)
