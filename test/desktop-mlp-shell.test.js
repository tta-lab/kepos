...
test('desktop context actions live behind a dedicated component boundary', async () => {
  ...
  assert.match(context, /createAdvancedSocialActionState\(/)
...
test('desktop error handling keeps raw exception detail advanced', async () => {
  ...
  assert.match(controller, /Could not read this Debug Home QR\./)
...
test('desktop primary panes expose short empty states before content arrives', async () => {
  ...
  assert.match(source, /<ListEmptyState[\s\S]*title='No messages yet'/)
  assert.match(source, /from '\.\.\/src\/direct-chat-empty-copy\.ts'/)
  assert.match(source, /const emptyCopy = getDirectChatEmptyCopy\(\{/)
  assert.match(source, /relationshipState/)
  assert.doesNotMatch(source, /function readProfileRelationshipState/)
...
test('desktop panes label live and durable surfaces', async () => {
  ...
  assert.match(source, /Write a friend request to introduce yourself\./)
  assert.match(source, /function FriendRequestComposer\(/)
  assert.match(source, /id='friendRequestForm'/)
  assert.match(source, /id='friendRequestInput'/)
  assert.match(source, /id='friendRequestSendButton'/)
  assert.match(source, /requestComposerState\.isVisible \? null : \(/)
...
test('desktop context actions disable unavailable joins and trust', async () => {
  ...
  assert.match(source, /createAdvancedSocialActionState\(/)
  assert.match(presenter, /ui\?\.setControls\(\{/)
  ...
  assert.match(source, /canUseManualHomeJoin: controls\.canUseManualHomeJoin/)
  assert.match(source, /canUseHomeQrJoin: controls\.canUseHomeQrJoin/)
  assert.match(source, /canUseTrustProfile: controls\.canUseTrustProfile/)
  assert.match(context, /<ActionButton[\s\S]*disabled=\{!actionState\.canJoinManualHome\}/)
  assert.match(context, /<ActionButton[\s\S]*disabled=\{!actionState\.canJoinHomeQr\}/)
  assert.match(context, /<ActionButton[\s\S]*disabled=\{!actionState\.canTrustProfile\}/)
