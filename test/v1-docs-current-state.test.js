...
test('project direction doc reflects the current V1 evidence and user loop', async () => {
  ...
  assert.match(direction, /as of 2026-07-03/)
  assert.match(direction, /`npm run v1:gate` has passed/)
  assert.match(direction, /966 Node tests/)
...
test('V1 docs map points agents to the current ready checklist and smoke recipes', async () => {
  ...
  assert.match(docs, /Active V1 architecture reset/)
...
test('V1 architecture reset treats complexity as duplicated product ownership', async () => {
  ...
  assert.match(reset, /request-target sends now use\s+`src\/friend-request-composer-state\.ts`/)
  assert.doesNotMatch(reset, /compatibility request composer path/)
...
test('V1 previous profile and DM first release plan demotes Home from social delivery', async () => {
  ...
  assert.match(plan, /Android backend profile startup now opens the owner's Treehole/)
  assert.match(plan, /does not join Home/)
  assert.match(
    plan,
...
test('V1 audit records DM security hardening evidence', async () => {
  ...
  assert.match(audit, /966 Node\s+tests/)
  assert.match(audit, /3a4ef061b7107d69451399e6d193a5477be8322e/)
...
test('V1 completion plan separates automated evidence from final smoke proof', async () => {
  ...
  assert.match(completionPlan, /966 Node tests/)
...
test('V1 active plan records source-level profile-social hardening as closed', async () => {
  ...
  assert.match(plan, /`549679100ae0534c87e456e80172edda8eda009e`/)
  assert.match(plan, /969 Node tests/)
  ...
  assert.match(plan, /passed and superseded by the current full non-device gate/)
  assert.match(plan, /`npm run v1:gate` passed on/)
  assert.match(plan, /`549679100ae0534c87e456e80172edda8eda009e`/)
  assert.match(plan, /passed with 62 tests on `4d20c34a0ef3db278d4517541f41ca78acac2cad`/)
  assert.match(plan, /it remains useful as a quick local recheck/)
  assert.match(plan, /No physical desktop\/Android smoke was run for either low-cost proof pass/)
  assert.match(
    plan,
