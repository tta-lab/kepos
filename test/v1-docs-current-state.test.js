...
test('V1 TypeScript boundary records current source shape and smoke policy', async () => {
  ...
  assert.match(boundary, /Node ESM \/ NodeNext/)
  assert.match(boundary, /emitted JavaScript extension/)
  assert.match(boundary, /extensionless local imports are acceptable/)
  assert.match(boundary, /Do not use `\.ts` import specifiers as the default style/)
  assert.match(boundary, /noEmit` with `allowImportingTsExtensions/)
  assert.match(boundary, /JavaScript\/MJS runtime glue/)
  assert.match(boundary, /Do\s+not churn them solely for style/)
