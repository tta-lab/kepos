...
test('normal error notices avoid raw exception text', async () => {
  ...
  assert.match(mobile, /Could not read this Debug Home QR\./)
  assert.match(desktop, /Could not join this home\. Trust this friend on this device first\./)
  assert.match(desktop, /Could not read this Debug Home QR\./)
...
test('desktop success notices avoid profile id snippets', async () => {
  ...
  const requestDelivery = await readFile(
    new URL('../src/profile-friend-request-delivery.ts', import.meta.url),
    'utf8'
  )
...
test('Android icon-only buttons expose accessible labels', async () => {
  ...
  for (const label of ['Leave home', 'Send home message', 'Post to Treehole', 'Send comment']) {
