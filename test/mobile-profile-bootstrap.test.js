...
import { createDmThreadListView } from '../src/dm-thread-list.ts'
import { saveDmThreadsToFileSystem } from '../src/dm-thread-storage.ts'
import {
...
test('mobile runtime profile restores trusted Chat rows and accepted threads after restart', async () => {
  ...
  const contactBook = trustContact(createContactBook({ ownerProfileId: localProfileId }), {
    alias: 'Ada',
    profileId: friendProfileId,
    trustedAt: 1000
  })
  const acceptedThread = acceptDmThread(
    createDmThread({
  ...
  await saveDmThreadsToFileSystem({
    baseUri: 'file:///app/',
    fileSystem,
    threads: [acceptedThread]
  })

  const profile = await loadMobileRuntimeProfile({
    createHomeRoomKey: () => 'e'.repeat(64),
    createIdentity: () => ({
      publicKey: localProfileId,
      secretKey: 'f'.repeat(128)
    }),
    fileSystem
  })
  const [row] = createDmThreadListView({
    contactBook: profile.contactBook,
    formatTime: (value) => `t:${value}`,
    shortenProfileId: (value) => value.slice(0, 6),
    threads: profile.dmThreads
  })

  assert.equal(profile.contactBook.contactsByProfileId.get(friendProfileId)?.alias, 'Ada')
  assert.deepEqual(profile.dmThreads, [acceptedThread])
  assert.deepEqual(profile.treeholePolicy, {
    ownerProfileId: localProfileId,
    revokedProfileIds: [],
    trustedProfileIds: [friendProfileId]
  })
  assert.deepEqual(
    {
      label: row.label,
