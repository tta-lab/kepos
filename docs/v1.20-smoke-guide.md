...
Current automated coverage proves lint, tests, typecheck, Android bundle creation, Expo Android JS bundle creation, and APK native-library alignment. Current device smoke covers Android scanner open/cancel and the debug two-device desktop/Android runtime path when the Android runtime is stable, with an optional Pear/Bare desktop variant for the same cross-device debug path. Physical QR remains release proof: the current desktop Large Profile QR and Large Home QR screen-to-camera path into Android trust and trusted-only home join must be rerun before declaring V1 complete.
...
treehole state fallback, message request acceptance, signed DM body exchange in both directions,
Android DM persistence across restart, post-restart DM delivery, and the desktop revoke path closing
the accepted DM receive path. The `:pear` variant keeps the same Android/debug setup while the
desktop side uses the bundled Pear/Bare worker instead of the deterministic in-process fallback. It
is useful CLI evidence for the live runtime, but physical QR smoke should still be repeated before
release builds.
