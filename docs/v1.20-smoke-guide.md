...
Current automated coverage proves lint, tests, typecheck, Android bundle creation, Expo Android JS bundle creation, and APK native-library alignment. Current device smoke covers Android scanner open/cancel and the debug two-device desktop/Android runtime path when the Android runtime is stable, with an optional Pear/Bare desktop variant for the same cross-device debug path. Physical QR release proof passed for the current desktop Large Profile QR and Large Home QR screen-to-camera path into Android trust and trusted-only home join.
...
The manual physical QR release proof can be staged with:
...
After Android reports trust, the script repeats the same flow for the desktop Large Home QR and
waits for Android to reach the connected Home view. The `:pear` variant uses the bundled desktop
...
Direct thread in both directions, Android DM persistence across restart, post-restart DM delivery,
and the desktop revoke path closing the accepted DM receive path. The `:pear` variant keeps the same
