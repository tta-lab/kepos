...
1. Extract desktop backend behavior behind a worker-shaped command/event boundary.
   ...
   `src/desktop-command-registry.ts`; Home, Direct Message, and Treehole now
   live in renderer-local runtime modules. ContactBook, QR, revoke orchestration,
   and local profile setup still live in `desktop/controller.js`.
...
3. Keep the current UI working through the new bridge.
   ...
   and Treehole runtime work through explicit modules.
