...
- This is not the Bare worker bridge yet. It is the renderer-facing bridge API
  and naming SSOT that the worker migration should preserve.
...
1. Extract desktop backend behavior behind a worker-shaped command/event boundary.
   Current status: started through `src/desktop-command-vocabulary.ts`; runtime
   code still lives in `desktop/controller.js`, but UI actions now pass through
...
3. Keep the current UI working through the new bridge.
   Current status: pending bridge, but `desktop/controller.js` preserves current
   behavior behind the React shell.
