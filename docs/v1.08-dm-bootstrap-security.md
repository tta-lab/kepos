...
- ContactBook clears an existing pending message request when the same profile is revoked
...
- add request accept and invite accept helpers: done in `src/message-request-acceptance.ts` and `src/dm-invite-acceptance.ts`
- ignored message requests now block repeat untrusted requests from the same
  profile, and request acceptance fails closed without a pending request
...
- keep DM message contents out of home room traffic: done for the product path;
  Home-carried `kepos.dm.body.v1` frames are gated behind an explicit debug
  fallback flag and are not sent or accepted by default
...
- reject invites whose signed recipient does not match the local profile before
  opening a thread
- bind request-bound invites to a matching local outgoing message request before
  allowing an untrusted sender to open a thread
- keep ContactBook invite policy trusted-sender-only; request-bound exceptions
  must be supplied by the platform runtime that owns local request history
- sign optional expiration and reject stale invites before accepting a thread
...
Status: implementation done in the shared model, platform wiring, and debug two-device runtime path.

The pure thread state model, storage adapters, desktop/Android setup wiring,
signed DM message record shape, durable per-thread signed message storage,
dedicated DM replication channel primitive, desktop/Android lifecycle wiring,
and trusted-contact recipient picker exist. `smoke:two-device:debug` and
`smoke:two-device:debug:pear` now prove signed DM exchange, Messages restart
persistence, post-restart Messages delivery, and desktop revoke closing the accepted Messages
receive path on a connected Pixel 7a dev-client setup. Manual physical
desktop-to-Android QR trust/home join also passed on the connected Pixel 7a
through the normal scanner path. Later contact polish has also landed: desktop
and mobile surface pending Messages/Contacts work, guide zero-contact Messages users
toward Contacts, keep raw recipient ids in Advanced, show product empty states
for Contacts, and keep trusted-friend metadata in Home terminology.

## V1 Rule

V1 should not present deterministic pairwise topic DM as the product architecture.

If a simpler path remains in code for smoke testing, it must be named and
treated as prototype-only. The Home-carried signed DM body fallback is now that
kind of path: it is disabled by default and must be enabled explicitly for debug
work.
