...
Status: implementation done in the shared model and platform wiring; debug two-device remains
release proof.

The pure thread state model, storage adapters, desktop/Android setup wiring, signed DM message record shape, durable per-thread signed message storage, dedicated DM replication channel primitive, desktop/Android lifecycle wiring, and trusted-contact recipient picker exist. `smoke:two-device:debug` is not yet passing on the current Android emulator/dev-client setup, so signed DM exchange, restart persistence, desktop revoke close path, and manual physical desktop-to-Android QR trust/home join remain release proof. Later contact polish has also landed: desktop and mobile surface pending Direct/People work, guide zero-contact Direct users toward People, keep raw recipient ids in Advanced, show product empty states for People, and keep trusted-friend metadata in Home terminology.
