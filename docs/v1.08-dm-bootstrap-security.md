...
The pure thread state model, storage adapters, desktop/Android setup wiring, signed DM message record shape, durable per-thread signed message storage, dedicated DM replication channel primitive, desktop/Android lifecycle wiring, and trusted-contact recipient picker exist. `smoke:two-device:debug` and `smoke:two-device:debug:pear` now prove signed DM exchange, restart persistence, post-restart delivery, and desktop revoke closing the accepted DM receive path on a connected Pixel 7a dev-client setup. Manual physical desktop-to-Android QR trust/home join also passed on the connected Pixel 7a through the normal scanner path. Later contact polish has also landed: desktop and mobile surface pending Direct/People work, guide zero-contact Direct users toward People, keep raw recipient ids in Advanced, show product empty states for People, and keep trusted-friend metadata in Home terminology.
