...
`smoke:two-device:debug` is not yet passing on the current Android emulator/dev-client setup, so
debug two-device remains release proof rather than current completion evidence.
