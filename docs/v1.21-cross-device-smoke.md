...
normal non-smoke path uses. Keep `smoke:desktop` as the faster deterministic fallback gate.
`smoke:desktop:contacts:pear` adds Profile QR trust, restart persistence, Direct message request
creation, Direct request restart display, and proof that the revoked trusted contact stays hidden
after restart on that same worker path.
...
desktop exposes one, then verifies bidirectional room chat, treehole state fallback, signed DM body
fallback, Android DM restart persistence, post-restart DM delivery, and revoke.
...
- Direct message request rows remain after app restart
