...
- Compatibility probes confirm the chosen crypto/encoding/encryption libraries work in Node and Bare Android bundle. Done for Node execution and Android bundle. Android runtime evidence now comes from `smoke:android`, `smoke:two-device:debug`, and `smoke:two-device:debug:pear`; physical screen-to-camera QR remains release proof.
