...
The system-level payloads still carry public keys and capabilities. Home
addresses belong to Home address payloads or post-trust descriptor delivery, not
the normal Profile QR path.
...
- scanner sees a friend-request target in Chat
...
- trusted users can create a real Chat thread
- Profile QR must not carry a Home descriptor in the normal V1 product path
- a signed Home descriptor can arrive later through advanced/debug Home QR or
...
5. B writes a request message and sends it.
6. A sees the request in Contacts.
7. A accepts or ignores the request.
8. If A accepts, both sides record trust for the relationship.
9. Both sides can open a durable Chat thread.
10. Either side can explicitly enter the trusted contact's Home when a signed
...
8. Add profile-owned Home descriptor refresh. Superseded: Profile QR must not
   carry a Home descriptor in the normal V1 product path. Descriptor refresh
   should use advanced/debug Home QR after trust or profile-level post-trust
   delivery.
