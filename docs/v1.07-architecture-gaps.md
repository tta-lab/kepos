...
- debug two-device now proves trusted home entry, treehole replication, DM setup, signed DM body exchange over the accepted message thread, Messages restart persistence, and desktop revoke closing the accepted Messages receive path in the UI/runtime on a connected Pixel 7a dev-client setup
- physical QR sub-proof passed for desktop Large Profile QR creating trust on Android, plus Advanced Large Home QR joining the trusted-only home after trust already exists
...
trusted home entry, treehole state fallback, signed DM body exchange over the accepted message
thread, Android Messages restart persistence, post-restart Messages delivery, and desktop revoke on a connected
...
- message requests from already trusted senders are ignored by ContactBook
- ignored message requests consume the sender's one untrusted request slot, and
  request acceptance requires pending ContactBook state before creating trust
- Android appends incoming message requests to Messages only after ContactBook accepts the request, so revoked senders do not leak into the Messages request UI
...
- Android syncs ContactBook-derived treehole policy into the Bare backend after
  trust, request accept, and revoke, so backend trust checks do not stay stuck
  on the join-time policy snapshot
- active desktop and Android treehole sessions accept live policy updates, so
  signed treehole state is recomputed after trust or revoke instead of waiting
  for the next treehole open
...
- Android scan action handling is factored into `src/mobile-qr-actions.ts` and covered by `test/mobile-qr-actions.test.js`
- debug two-device remains live runtime proof for treehole, room chat, signed DM body exchange over the accepted message thread, restart persistence, desktop revoke, and Android scanner open/cancel; it does not replace normal Profile QR request/ignore/recover/accept release proof
...
- debug two-device remains live runtime proof that desktop revoke removes the trusted contact and closes the accepted Messages receive path; final release proof still needs the normal Profile QR -> request -> ignore -> allow -> request -> accept path
...
- debug two-device remains live runtime proof for signed DM body exchange over the accepted message thread, Android Messages persistence across restart, post-restart Messages delivery, and desktop revoke closing the accepted Messages receive path; final release proof still needs the normal Profile QR -> request -> ignore -> allow -> request -> accept path
...
- Android Messages body persistence across restart
- post-restart signed Messages delivery
- desktop revoke removing Android from trusted contacts
- desktop revoke closing the accepted Messages receive path
...
QR sub-proof also passed for the normal desktop Large Profile QR path into Android
trust, plus the Advanced Large Home QR path into Android home join after trust
already exists.
