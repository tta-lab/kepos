...
- debug two-device remains release proof for trusted home entry, treehole replication, DM setup, signed DM body exchange, restart persistence, and desktop revoke closing the accepted DM receive path in the UI/runtime
...
the shared domain path. The debug two-device script is the intended live transport and UI wiring
release proof for trusted home entry, treehole replication, and signed DM, but
`smoke:two-device:debug` is not yet passing on the current Android emulator/dev-client setup.
...
- backend boundary tests and the integrated V1 model smoke cover the policy decision on the shared model; debug two-device remains release proof for the live connection
...
This remains debug-path release proof for the live transport, and `smoke:two-device:debug` is not
yet passing on the current Android emulator/dev-client setup. The screen-to-camera QR proof remains
open for the normal desktop Large Profile QR and Large Home QR path into Android trust and home join.
