...
- debug two-device now proves trusted home entry, treehole replication, DM setup, signed DM body exchange over the accepted message thread, Chat restart persistence, and desktop revoke closing the accepted Chat receive path in the UI/runtime on a connected Pixel 7a dev-client setup
- physical QR sub-proof passed for desktop Large Profile QR creating trust on Android, plus Advanced Large Debug Home QR joining the trusted-only home after trust already exists
...
thread, Android Chat restart persistence, post-restart Chat delivery, and desktop revoke on a connected
...
- Android appends incoming message requests to Chat only after ContactBook accepts the request, so revoked senders do not leak into the Chat request UI
...
- debug two-device remains live runtime proof that desktop revoke removes the trusted contact and closes the accepted Chat receive path; final release proof still needs the normal Profile QR -> request -> ignore -> allow -> request -> accept path
...
- debug two-device remains live runtime proof for signed DM body exchange over the accepted message thread, Android Chat persistence across restart, post-restart Chat delivery, and desktop revoke closing the accepted Chat receive path; final release proof still needs the normal Profile QR -> request -> ignore -> allow -> request -> accept path
...
- Android Chat body persistence across restart
- post-restart signed Chat delivery
- desktop revoke removing Android from trusted contacts
- desktop revoke closing the accepted Chat receive path
...
trust, plus the Advanced Large Debug Home QR path into Android home join after trust
already exists.
...
- Desktop Large Debug Home QR scanned by Android and joined the trusted-only home after trust existed.
